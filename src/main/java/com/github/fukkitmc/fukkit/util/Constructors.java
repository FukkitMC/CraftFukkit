...
public class Constructors {
	...
	public static LevelGeneratorType newLevelGeneratorType(int id, String name) throws IllegalAccessException,
	...
	public static ServerWorld newWorld(MinecraftServer server, Executor workerExecutor,
	                                   WorldSaveHandler worldSaveHandler, LevelProperties properties,
	                                   DimensionType dimensionType, Profiler profiler,
	...
	public static SecondaryServerWorld newWorld2nd(ServerWorld sworld, MinecraftServer server, Executor workerExecutor
	, WorldSaveHandler worldSaveHandler, DimensionType dimensionType, Profiler profiler,
	                                               WorldGenerationProgressListener worldGenerationProgressListener,
	                                               ChunkGenerator generator, World.Environment environment) {
		...
		return new Slot(inventory, 39 - n, 8, 8 + n * 18) {
			...
			public boolean canTakeItems(PlayerEntity playerEntity) {
				...
				return (itemStack.isEmpty() || playerEntity.isCreative() || !EnchantmentHelper
				                                                             .hasBindingCurse(itemStack)) && super
				                                                                                             .canTakeItems(playerEntity);
			}
		};
	}
