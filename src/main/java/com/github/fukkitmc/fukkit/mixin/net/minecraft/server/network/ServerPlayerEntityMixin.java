...
public abstract class ServerPlayerEntityMixin extends PlayerEntityMixin {
	...
	private void fukkit_init(MinecraftServer server, ServerWorld world, GameProfile profile, ServerPlayerInteractionManager interactionManager, CallbackInfo ci) {
	...
	public final BlockPos fukkit$getSpawnPoint(ServerWorld worldserver) {
		...
		if (worldserver.dimension.isNether() && worldserver.getLevelProperties().getGameMode() != net.minecraft.world.GameMode.ADVENTURE) {
			int i = Math.max(0, this.server.getSpawnRadius(worldserver));
			int j = MathHelper.floor(worldserver.getWorldBorder().getDistanceInsideBorder(blockposition.getX(), blockposition.getZ()));
			...
			for (int j1 = 0; j1 < k; ++j1) {
				...
				int l1 = k1 % (i * 2 + 1);
				int i2 = k1 / (i * 2 + 1);
				BlockPos blockposition1 = worldserver.getDimension().getTopSpawningBlockPosition(blockposition.getX() + l1 - i, blockposition.getZ() + i2 - i, false);
	...
	@Inject (method = "readCustomDataFromTag", at = @At (value = "INVOKE", target = "Lnet/minecraft/server/network/ServerPlayerEntity;isSleeping()Z"))
	private void fukkit_readExtra(CompoundTag tag, CallbackInfo ci) {
		((ServerPlayerEntityAccess) this).getBukkit().readExtraData(tag);
	}

	@Redirect (method = "writeCustomDataToTag", at = @At (value = "INVOKE", target = "Lnet/minecraft/entity/Entity;hasPlayerRider()Z"))
		...
		if (world == null) {
			...
			if (this.spawnWorld != null && !"".equals(this.spawnWorld)) {
				...
				if (cworld != null && this.getSpawnPosition() != null) {
					...
					pos = PlayerEntity.findRespawnPosition(cworld.getHandle(), this.getSpawnPosition(), false).orElse(null);
				}
			}
	...
	@Redirect (method = "playerTick", at = @At (value = "INVOKE", target = "Lnet/minecraft/server/network/ServerPlayerEntity;getHealth()F", ordinal = 1))
	private float fukkit_scaleHealth(ServerPlayerEntity entity) {
		return ((CraftPlayer) this.fukkit$getBukkit()).getScaledHealth();
	}

	@Inject (method = "playerTick", at = @At (value = "FIELD", target = "Lnet/minecraft/server/network/ServerPlayerEntity;experienceLevel:I", ordinal = 0))
	...
	// imperfect inject
	@Inject (method = "playerTick", at = @At (value = "FIELD", target = "Lnet/minecraft/server/network/ServerPlayerEntity;age:I"))
	private void fukkit_oldLevel(CallbackInfo ci) {
		if (this.oldLevel == -1) this.oldLevel = this.experienceLevel;

		if (this.oldLevel != this.experienceLevel) {
			CraftEventFactory.callPlayerLevelChangeEvent(((WorldAccess) this.world).getBukkitServer().getPlayer((ServerPlayerEntity) (Object) this), this.oldLevel, this.experienceLevel);
			this.oldLevel = this.experienceLevel;
		}
	}

	@Redirect (method = "updateScores", at = @At (value = "INVOKE", target = "Lnet/minecraft/scoreboard/Scoreboard;forEachScore(Lnet/minecraft/scoreboard/ScoreboardCriterion;Ljava/lang/String;Ljava/util/function/Consumer;)V"))
	private void fukkit_useBukkitScores(Scoreboard scoreboard, ScoreboardCriterion criterion, String player, Consumer<ScoreboardPlayerScore> action) {
	...
	private void fukkit_remSpecTarget(Scoreboard scoreboard, ScoreboardCriterion criterion, String player, Consumer<ScoreboardPlayerScore> action) {
		this.setCameraEntity((Entity) (Object) this);
		((WorldAccess) this.world).getBukkitServer().getScoreboardManager().getScoreboardScores(criterion, player, action);
	}

	// two birds 1 stone
	@Redirect (method = "updateKilledAdvancementCriterion", at = @At (value = "INVOKE", target = "Lnet/minecraft/scoreboard/Scoreboard;forEachScore(Lnet/minecraft/scoreboard/ScoreboardCriterion;Ljava/lang/String;Ljava/util/function/Consumer;)V"))
	private void fukkit_useBukkitScore(Scoreboard scoreboard, ScoreboardCriterion criterion, String player, Consumer<ScoreboardPlayerScore> action) {
		((WorldAccess) this.world).getBukkitServer().getScoreboardManager().getScoreboardScores(criterion, player, action);
	}

	@Redirect (method = "updateScoreboardScore", at = @At (value = "INVOKE", target = "Lnet/minecraft/scoreboard/Scoreboard;forEachScore(Lnet/minecraft/scoreboard/ScoreboardCriterion;Ljava/lang/String;Ljava/util/function/Consumer;)V"))
	private void fukkit_useBukkitScore0(Scoreboard scoreboard, ScoreboardCriterion criterion, String player, Consumer<ScoreboardPlayerScore> action) {
		((WorldAccess) this.world).getBukkitServer().getScoreboardManager().getScoreboardScores(criterion, player, action);
	}

	@Redirect (method = "isPvpEnabled", at = @At (value = "INVOKE", target = "Lnet/minecraft/server/MinecraftServer;isPvpEnabled()Z"))
	...
	public Entity fukkit$changeDimension(DimensionType type, PlayerTeleportEvent.TeleportCause cause) {
		// CraftBukkit end
		if (this.isSleeping()) return (Entity) (Object) this; // CraftBukkit - SPIGOT-3154
		// this.worldChangeInvuln = true; // CraftBukkit - Moved down and into PlayerList#changeDimension
		DimensionType currentDimensionType = this.dimension;

		if (((DimensionTypeAccess) currentDimensionType).getType() == DimensionType.THE_END && ((DimensionTypeAccess) type).getType() == DimensionType.OVERWORLD) { // CraftBukkit - getType()
			...
			Location exit = (newWorld == null) ? null : new Location(((WorldAccess) newWorld).getBukkit(), currentX, currentY, currentZ, currentYaw, currentPitch);
			PlayerPortalEvent event = new PlayerPortalEvent((Player) this.fukkit$getBukkit(), enter, exit, cause, 128, true, ((DimensionTypeAccess) type).getType() == DimensionType.THE_END ? 0 : 16);
			...
			if (((DimensionTypeAccess) type).getType() == DimensionType.THE_END) { // CraftBukkit - getType()
				...
				if (event.getCanCreatePortal()) {
					// CraftBukkit end
					org.bukkit.craftbukkit.util.BlockStateListPopulator blockList = new org.bukkit.craftbukkit.util.BlockStateListPopulator(newWorld); // CraftBukkit - Use BlockStateListPopulator
					for (int zOffset = -2; zOffset <= 2; ++zOffset) {
						for (int xOffset = -2; xOffset <= 2; ++xOffset) {
							for (int yOffset = -1; yOffset < 3; ++yOffset) {
								...
								blockList.setBlockState(new BlockPos(blockX, blockY, blockZ), placeObsidian ? Blocks.OBSIDIAN.getDefaultState() : Blocks.AIR.getDefaultState(), 3); // CraftBukkit
							}
						}
					}
					...
					org.bukkit.event.world.PortalCreateEvent portalEvent = new org.bukkit.event.world.PortalCreateEvent((List<BlockState>) (List) blockList.getList(), bukkitWorld, (Player) this.fukkit$getBukkit(), org.bukkit.event.world.PortalCreateEvent.CreateReason.END_PLATFORM);
				...
				BlockPattern.TeleportTarget portalShape = ((PortalForcerAccess) newWorld.getPortalForcer()).findAndTeleport((ServerPlayerEntity) (Object) this, exitPosition, tempYaw, event.getSearchRadius(), true);
				if (portalShape == null && event.getCanCreatePortal()) {
					if (((PortalForcerAccess) newWorld.getPortalForcer()).createPortal((ServerPlayerEntity) (Object) this, exitPosition, event.getCreationRadius())) { // Only check for new portal if creation succeeded
						portalShape = ((PortalForcerAccess) newWorld.getPortalForcer()).findAndTeleport((ServerPlayerEntity) (Object) this, exitPosition, tempYaw, event.getSearchRadius(), true);
					}
				}
			...
			PlayerTeleportEvent tpEvent = new PlayerTeleportEvent((Player) this.fukkit$getBukkit(), enter, exit, cause);
			...
			this.networkHandler.sendPacket(new PlayerRespawnS2CPacket(((DimensionTypeAccess) newWorld.dimension.getType()).getType(), LevelProperties.sha256Hash(this.world.getLevelProperties().getSeed()), this.world.getLevelProperties().getGeneratorType(), this.interactionManager.getGameMode()));
			...
			((ServerPlayNetworkHandlerAccess) this.networkHandler).teleport(exit); // CraftBukkit - use internal teleport without event
			this.networkHandler.syncWithPlayerPosition(); // CraftBukkit - sync position after changing it (from PortalTravelAgent#findAndteleport)
			...
			// CraftBukkit start
			PlayerChangedWorldEvent changeEvent = new PlayerChangedWorldEvent((Player) this.fukkit$getBukkit(), ((ServerWorldAccess) currentWorld).getBukkit());
	...
	@ModifyVariable (method = "openContainer(Lnet/minecraft/container/NameableContainerFactory;)Ljava/util/OptionalInt;", at = @At (value = "INVOKE_ASSIGN", target = "Lnet/minecraft/container/NameableContainerFactory;createMenu(ILnet/minecraft/entity/player/PlayerInventory;Lnet/minecraft/entity/player/PlayerEntity;)Lnet/minecraft/container/Container;"))
	private Container fukkit_invOpenHook(Container container, NameableContainerFactory containerFactory) {
		if (container != null) {
			...
			if (container == null && !cancelled) { // for your reference md5.. booleans are not in fact passed by reference..
				if (containerFactory instanceof Inventory)
					...
					((BukkitChestDoubleInventory) containerFactory).inventory.onInvClose((ServerPlayerEntity) (Object) this);
				}
	...
	@Redirect (method = "openContainer(Lnet/minecraft/container/NameableContainerFactory;)Ljava/util/OptionalInt;", at = @At (value = "INVOKE", target = "Lnet/minecraft/container/Container;addListener(Lnet/minecraft/container/ContainerListener;)V"))
	private void fukkit_moveDown(Container container, ContainerListener listener) {}

	@Redirect (method = "openContainer(Lnet/minecraft/container/NameableContainerFactory;)Ljava/util/OptionalInt;", at = @At (value = "INVOKE", target = "Lnet/minecraft/server/network/ServerPlayNetworkHandler;sendPacket(Lnet/minecraft/network/Packet;)V"))
	private void fukkit_containerTitle(ServerPlayNetworkHandler handler, Packet<?> packet) {}

	@Inject (method = "openContainer(Lnet/minecraft/container/NameableContainerFactory;)Ljava/util/OptionalInt;", at = @At (value = "RETURN", ordinal = 0), slice = @Slice (from = @At (value = "INVOKE", target = "Lnet/minecraft/container/Container;addListener(Lnet/minecraft/container/ContainerListener;)V")), locals = LocalCapture.CAPTURE_FAILHARD)
	private void fukkit_containerTitle(NameableContainerFactory nameableContainerFactory, CallbackInfoReturnable<OptionalInt> cir, Container container) {
		this.networkHandler.sendPacket(new OpenContainerS2CPacket(container.syncId, container.getType(), ((ContainerAccess) container).getTitle()));
	...
	@Redirect (method = "openHorseInventory", at = @At (value = "NEW", target = "net/minecraft/container/HorseContainer"))
	private HorseContainer fukkit_horseContainer(int syncId, PlayerInventory playerInventory, Inventory inventory, HorseBaseEntity horseBaseEntity) {
		return null; // pranked
	}

	@Inject (method = "openHorseInventory", at = @At (value = "INVOKE", target = "Lnet/minecraft/container/Container;addListener(Lnet/minecraft/container/ContainerListener;)V"))
	...
	@Redirect (method = "openHorseInventory", at = @At (value = "INVOKE", target = "Lnet/minecraft/server/network/ServerPlayerEntity;incrementContainerSyncId()V"))
	...
	private void fukkit_updateResult(Container container, DefaultedList<ItemStack> defaultedList, CallbackInfo ci) {
		if (java.util.EnumSet.of(InventoryType.CRAFTING, InventoryType.WORKBENCH).contains(((ContainerAccess) container).getBukkitView().getType())) {
			this.networkHandler.sendPacket(new ContainerSlotUpdateS2CPacket(container.syncId, 0, container.getSlot(0).getStack()));
		}
	}
	...
	@Redirect (method = {"increaseStat", "resetStat"}, at = @At (value = "INVOKE", target = "Lnet/minecraft/scoreboard/Scoreboard;forEachScore(Lnet/minecraft/scoreboard/ScoreboardCriterion;Ljava/lang/String;Ljava/util/function/Consumer;)V"))
	private void fukkit_bukkitScores(Scoreboard scoreboard, ScoreboardCriterion criterion, String player, Consumer<ScoreboardPlayerScore> action) {
		((WorldAccess) this.world).getBukkitServer().getScoreboardManager().getScoreboardScores(criterion, player, action);
	}
	...
	@Redirect (method = "copyFrom", at = @At (value = "INVOKE", target = "Lnet/minecraft/server/network/ServerRecipeBook;copyFrom(Lnet/minecraft/recipe/book/RecipeBook;)V"))
	...
	private void fukkit_gameModeChangeEvent(GameMode gameMode, CallbackInfo ci) {
		...
		PlayerGameModeChangeEvent event = new PlayerGameModeChangeEvent((Player) this.fukkit$getBukkit(), org.bukkit.GameMode.getByValue(gameMode.getId()));
	...
	private void fukkit_changeHandEvent(ClientSettingsC2SPacket packet, CallbackInfo ci) {
		if (this.getMainArm() != packet.getMainArm()) {
			PlayerChangedMainHandEvent event = new PlayerChangedMainHandEvent((Player) this.fukkit$getBukkit(), this.getMainArm() == Arm.LEFT ? MainHand.LEFT : MainHand.RIGHT);
			((WorldAccess) this.server).getBukkitServer().getPluginManager().callEvent(event);
		}
		if (!this.clientLanguage.equals(packet.getLanguage())) {
			PlayerLocaleChangeEvent event = new PlayerLocaleChangeEvent((Player) this.fukkit$getBukkit(), packet.getLanguage());
			((WorldAccess) this.server).getBukkitServer().getPluginManager().callEvent(event);
		}
	...
	private void fukkit_teleCause(ServerPlayerEntity entity, double destX, double destY, double destZ) {
		((ServerPlayNetworkHandlerAccess) this.networkHandler).teleportRequest(this.cameraEntity.getX(), this.cameraEntity.getY(), this.cameraEntity.getZ(), this.yaw, this.pitch, PlayerTeleportEvent.TeleportCause.SPECTATE);
	}
	...
	public void fukkit$teleport(ServerWorld world, double x, double y, double z, float yaw, float pitch, org.bukkit.event.player.PlayerTeleportEvent.TeleportCause cause) {
		// CraftBukkit end
		this.setCameraEntity((ServerPlayerEntity) (Object) this);
		this.stopRiding();
		((Player) this.fukkit$getBukkit()).teleport(new Location(((WorldAccess) world).getBukkit(), x, y, z, yaw, pitch), cause);
		// CraftBukkit end
	}
	...
	public void resetPlayerWeather() {
		this.weather = null;
		this.setPlayerWeather(this.world.getLevelProperties().isRaining() ? WeatherType.DOWNFALL : WeatherType.CLEAR, false);
	}

	@Override
	public String toString() {
		return super.toString() + "(" + this.getEntityName() + " at " + this.getX() + "," + this.getY() + "," + this.getZ() + ")";
	}
