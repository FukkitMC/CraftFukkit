...
public abstract class AbstractPressurePlateBlockMixin {
	@Redirect (method = "updatePlateState", at = @At (value = "INVOKE",
	                                                  target = "Lnet/minecraft/block/AbstractPressurePlateBlock;" +
	                                                           "getRedstoneOutput(Lnet/minecraft/world/World;" +
	                                                           "Lnet/minecraft/util/math/BlockPos;)I",
	                                                  ordinal = 0))
	public int fukkit_blockRedstoneEvent(AbstractPressurePlateBlock block, World world, BlockPos pos, World world2,
	                                     BlockPos pos2, BlockState blockState, int rsOut) {
		...
		if ((rsOut > 0) ^ (i > 0)) {
			BlockRedstoneEvent event = new BlockRedstoneEvent(bukkit.getBlockAt(pos.getX(), pos.getY(), pos
			                                                                                            .getZ()), rsOut, i);
